...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	lat, _ := strconv.ParseFloat(r.FormValue("lat"), 64) //read form, different form search
	...
	p := &Post{
		...
		Face: 0.0, // update url and type  later
	}
	...
	suffix := filepath.Ext(header.Filename) //if is video or image
	...
	id := uuid.New()
	attrs, err := saveToGCS(file, id) //save  image to gcs
	if err != nil {
		http.Error(w, "Failed to save image to GCS", http.StatusInternalServerError)
		fmt.Printf("Failed to save image to GCS %v\n", err)
		return
	}
	p.Url = attrs.MediaLink // the medialink returned
	if p.Type == "image" {
...
func saveToGCS(r io.Reader, objectName string) (*storage.ObjectAttrs, error) {
	...
	if _, err := bucket.Attrs(ctx); err != nil { // if bucket exist
		return nil, err
	}
